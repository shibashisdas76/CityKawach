...
export const HealthTelemetryPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl border border-slate-200/80 shadow-saasable gap-4">
        <div>
        ...
        <button
          ...
          disabled={refreshing}
          className="px-4.5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs flex items-center space-x-2 transition shadow-md shadow-indigo-600/20"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 text-xs">
        <div className="saasable-card p-5 space-y-2">
        ...
        <div className="saasable-card p-5 space-y-2">
          ...
          <p className="text-2xl font-extrabold text-indigo-600 tracking-tight">28%</p>
      ...
      <div className="saasable-card overflow-hidden">
        <div className="p-5 border-b border-slate-100 font-extrabold text-slate-900 text-xs flex justify-between items-center tracking-tight">
          ...
          <table className="w-full text-left text-xs font-mono border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {cameras.map((cam) => {
                ...
                return (
                  <tr key={cam.id} className="hover:bg-slate-50/50 transition">
                    <td className="py-3.5 px-4 text-indigo-600 font-bold">{cam.camera_id}</td>
                    ...
                    <td className="py-3.5 px-4 text-right font-sans">
                      <Link
                        to={`/cameras/${cam.id}`}
                        className="px-3 py-1 rounded-lg bg-slate-100 text-indigo-600 hover:bg-indigo-50 font-bold border border-slate-200 text-[11px] inline-flex items-center gap-1 transition"
                      >
                        <span>Telemetry</span>
